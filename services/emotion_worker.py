...
async def run_emotion_worker(
    ...
    try:
        while True:
            ...
            # Note: get_speech_emotion ALREADY gracefully handles pcm < MIN_AUDIO_SAMPLES by
            # returning neutral, so we shouldn't block text fusion if audio is short.
            has_enough_audio = rolling_buffer.size >= MIN_BUFFER_SAMPLES
            trigger_audio = (new_pcm.size > 0 and has_enough_audio)
            
            if (trigger_audio or has_new_text) and (current_time - last_inference_time > 1.0):
                ...
                if latest_transcribed_text:
                    ...
                    fused_text = latest_transcribed_text
                ...
                final_emotion["speech_text"] = fused_text
                final_emotion["text_message"] = fused_text
